...
<%
	try
	{
		if(request.getParameter("mobilenumber")!=null)
		{
			...
			if(status==1)
			{
				response.sendRedirect("viewstaffdetails.jsp?msg=Staff  Login Deactivated");
			}
			else
			{
				response.sendRedirect("viewstaffdetails.jsp?msg=Staff  Login Activated");
			}
...
<%
	rs=st.executeQuery("select * from tblstaffregister where usertype=2 order by firstname");
	int sl=1;
	while(rs.next())
	{
		%>
